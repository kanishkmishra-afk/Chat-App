    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white  ${selectedUser? 'max-md:hidden':''}`}>
      ...
      <div className='flex flex-col'>
        {userDummyData.map((user,index)=>(
         <div  onClick={()=>(setSelectedUser(user))} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm false ${selectedUser?._id===user._id && 'bg-[#282142]/50'}`}>
