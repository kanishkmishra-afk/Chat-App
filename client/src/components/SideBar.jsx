...
const SideBar = () => {
  ...
  const filterUser=input?users.filter((user)=>user.fullName.toLowerCase().includes(input.toLowerCase())):users;
    ...
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white  ${selectedUser? 'max-md:hidden':''}`}>
      <div className='pb-5'>
        ...
        <img src={assets.menu_icon} alt="menu" className='max-h-5 cursor-pointer '/>
          <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-[#282142] border border-gray-600 text-gray-100 hidden group-hover:block py-2 group'>
      ...
      <div className='flex flex-col'>
        {filterUser.map((user,index)=>(
         <div  onClick={()=>{setSelectedUser(user); setUnseenMessage(prev=>({...prev,[user._id]:0}))}} key={index} className={`relative flex items-center gap-2 p-2 pl-4 rounded cursor-pointer max-sm:text-sm false ${selectedUser?._id===user._id && 'bg-[#282142]/50'}`}>
           <img className='w-[35px] aspect-[1/1] rounded-full' src={user.profilePic || user} alt="userImage" />
